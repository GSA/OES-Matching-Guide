...
to remove the influence of the covariates?  We have two ways to answer those questions.
...
For example, we could make a plot to show this information within sets versus raw differences from the fullmatch.

```{r echo=FALSE, out.width=".9\\textwidth"}
library(gridExtra)
bpfm1 <- ggplot(dat, aes(x = fm1, y = age)) +
  geom_boxplot() +
  stat_summary(fun = mean, geom = "point", shape = 20, size = 3, color = "red", fill = "red")
dat$nostrata <- rep(1, 45)
bporig <- ggplot(dat, aes(x = nostrata, y = age)) +
  geom_boxplot() +
  stat_summary(
    fun = mean, geom = "point",
...
```{r echo=FALSE, out.width=".9\\textwidth"}
bppm1 <- ggplot(dat, aes(x = pm1, y = age)) +
  geom_boxplot() +
  stat_summary(fun = mean, geom = "point", shape = 20, size = 3, color = "red", fill = "red")
...
Are we comfortable with these differences? (Using the full match)

```{r sdiffs, echo=FALSE}
rawmndiffs <- with(dat, mean(age[nhTrt == 1]) - mean(age[nhTrt == 0]))
setdiffsfm1 <- dat %>%
  group_by(fm1) %>%
  summarize(
    mneddiffs =
...
Are we comfortable with these differences? (Using the pairmatch)
...
print(setdiffspm1, n = 100)
quantile(abs(setdiffspm1$diffs), seq(0, 1, .1))
```
...
```{r xbhs2, echo=TRUE}
xbfm1 <- balanceTest(nhTrt ~ age + strata(fm1) + strata(pm1) + strata(fm2), data = dat)
xbfm1$results[, , ]
xbfm1$overall
```
...
```{r xbhs2, echo=TRUE}
xbfm1 <- balanceTest(nhTrt ~ age + strata(fm1), data = dat)
xbfm1$results[, , ]
xbfm1$overall
```
...
setmeanDiffs <- dat %>%
  group_by(fm1) %>%
  summarise(
    diffAboveHS = mean(age[nhTrt == 1]) - mean(age[nhTrt == 0]),
    nb = n(),
    nTb = sum(nhTrt),
    nCb = sum(1 - nhTrt),
...
xb_1dim <- balanceTest(nhTrt ~ age + strata(fm1) + strata(pm1) + strata(fm2) + strata(fm3), data = dat)
...
balfmla <- reformulate(c("nhPopD", "age"), response = "nhTrt")
...
absdist <- match_on(tmpHom03, z = dat$nhTrt, data = dat)
absdist[1:3, 1:3]
quantile(as.vector(absdist), seq(0, 1, .1))
...
```{r}
dist2 <- psdist + exactMatch(nhTrt ~ classLowHi, data = dat)
summary(dist2)
## or mhdist <- match_on(balfmla,within=exactMatch(nhTrt~classLowHi,data=dat),data=dat,method="rank_mahalanobis")
...
```{r}
ftable(Class = dat$classLowHi, Trt = dat$nhTrt, fmEx1, col.vars = c("Class", "Trt"))
```
