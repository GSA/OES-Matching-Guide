...
and scaling, see the Rosenbaum chapter 9.X on this); and `ps_dist` which
...
covs <- c("sso", "age", "sex", "race", "edu", "grade", "pay", "tenure")
...
1, first set". And in our case, here we only have one overall group.

### How to assess a matched design?

Did either of those designs do a good job? Should we feel confident using them to remove the influence of the covariates?  We can answer that in two ways:
 - We can claim that our design 
...
```{r xbhs2, echo=TRUE}
xbfm1 <- balanceTest(nhTrt ~ nhAboveHS + strata(fm1) + strata(pm1) + strata(fm2), data = meddat)
xbfm1$results[, , ]
xbfm1$overall
```
...
fm3 <- fullmatch(absdist + caliper(absdist, .08), data = meddat)
summary(fm3, min.controls = 0, max.controls = Inf)

xb_1dim <- balanceTest(nhTrt ~ nhAboveHS + strata(fm1) + strata(pm1) + strata(fm2) + strata(fm3), data = meddat)
