...
<!-- TODO:  START HERE. Something weird about the pay variable in independence_test -->
...
## trtbin ~ sso + age + sexF + sexM + raceB + raceW + edub + educ + edud + edue + eduf + edug + eduh + grade + pay + tenure + preY


## The version using asymptotic approximations to do the randomization inference
balfmla_new_for_comparison <- trtbin ~ pay #sso + age +sexF + sexM + raceB + raceW + edub + edud + edue +eduf +eduh + grade + tenure + preY 
...
tmp_fmla <- reformulate(names(datmm)[!(names(datmm) %in% c("trtbin","trtbinF","fm1"))],response="trtbin")

## The version using asymptotic approximations to do the randomization inference
bt_fm1a <- balanceTest(stats::update(tmp_fmla,.~.-1+strata(fm1)),data=datmm)
bt_fm1$overall["fm1",]
xb_fm1 <- xBalance(tmp_fmla,strata=list(fm1=~fm1),data=datmm,report="all")
xb_fm1$overall["fm1",]
...
statistic(d2_coin_test_asymp,"linear")
#   sso   age sexF sexM raceB raceW edub educ edud edue eduf edug eduh grade      pay tenure  preY
#  1874 16327  169  165    44   278   53   48   13   60   33   20   30  1149 26306102   3328 101.8
statistic(d2_coin_test_asymp,"standardized")
#      sso   age    sexF   sexM  raceB raceW    edub  educ  edud  edue   eduf    edug  eduh   grade pay tenure  preY
#  -0.8073 11.02 -0.3581 0.3581 -3.372 4.092 -0.3875 1.465 1.151 1.197 -2.305 -0.3037 -2.42 -0.9319   1 -1.027 12.52

cbind(xb_fm1$results[,c("adj.diff","std.diff","z"),],matrix(statistic(d2_coin_test_asymp,"standardized"),ncol=1))
bt_fm1a$results[,c("adj.diff","std.diff","z"),]
...
tmp <- balanceTest(pr ~ date + t1 + t2 + cap + ne + ct + bw + cum.n + strata(pt),data=nuclearplants)
nuclearplants$prF <- factor(nuclearplants$pr)
nuclearplants$ptF <- factor(nuclearplants$pt)
tmp_coin <- independence_test(date + t1 + t2 + cap + ne + ct + bw + cum.n ~ prF | ptF, teststat="quadratic",data=nuclearplants)
